...
目前系統部分功能仍處於開發或優化階段，具體狀態如下：

| 功能模組 | 目前狀態 | 說明與優化方向 |
| :--- | :--- | :--- |
| **System Metrics** | 🎲 前端模擬數據 | 儀表板數據目前為模擬生成。需後端開發「資源採樣服務」以獲取真實 CPU/RAM 數據。 |
| **Live Console** | � HTTP Polling | 目前使用 HTTP 輪詢 (高延遲)。建議未來升級為 WebSocket 連線以實現即時日誌串流。 |
| **Process Monitor** | ❌ 無 (無法看 CPU/RAM) | 目前無法監控 Java 程序的資源使用量。需後端實作 Java Process 監控模組。 |
| **File Manager** | ❌ 無 (只有 Properties) | 僅支援 `server.properties` 編輯。需開發完整的檔案管理模組以支援檔案上傳/下載/編輯。 |
