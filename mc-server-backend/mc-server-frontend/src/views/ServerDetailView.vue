...
const handleStart = async () => {
  try {
    ...
    // Ignore already running error which might return 500 or 400
    if (err.response && err.response.data && err.response.data.error === 'server already running') {
       fetchServerDetail();
       return;
    }
    message.error('啟動失敗: ' + (err.response?.data?.error || '未知錯誤'));
  }
};
...
const handleStop = async () => {
  try {
    ...
    message.error('停止失敗: ' + (err.response?.data?.error || '未知錯誤'));
  }
};
            ...
            <n-card title="BACKUPS" class="backup-card fade-in-up" style="animation-delay: 0.2s;">
              ...
              <div class="backup-list">
                ...
                <div v-for="b in backups" :key="b.name" class="backup-item">
                  ...
                  <n-button size="tiny" ghost type="primary">Restore</n-button>
                </div>
              </div>
            </n-card>
          </n-space>
